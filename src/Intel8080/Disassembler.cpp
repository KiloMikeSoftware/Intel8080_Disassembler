...
int Disassembler::DisassembleOpCode(uint8_t * buffer, int pc)
{
	uint8_t * opcode = &buffer[pc];		//get a reference to the current memory location the program counter is at
	...
	std::cout << std::hex << pc;		//Start by printing the current address of the program counter

										//This switch iterates over all 256 opcodes.
	switch (*opcode)
	{
	case 0x00: NOP_NoOperation_0x00(opcode, cycles, pc); break;	//NOP - No Operation.
	case 0x01: break;
	case 0x02: break;
	case 0x03: break;
	case 0x04: break;
